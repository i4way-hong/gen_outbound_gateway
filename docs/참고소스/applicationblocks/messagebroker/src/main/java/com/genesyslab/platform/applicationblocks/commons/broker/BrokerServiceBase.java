...
public class BrokerServiceBase<T>
    ...
    @Deprecated
    protected BrokerServiceBase(final int capacity) {
        this();
    }
     ...
     * Default implementation calls subscribers' processing/handling methods of to process publishing event.
     *
     * @param obj Object that is used for a publishing event processing
     */
    protected void onPublish(final T obj) {
        notify(obj);
    }
     ...
     * This should be done with proper synchronization. This method supports that synchronization.
     * It uses addAll() method of Collection interface to populate new list with old values.
     *
     * @param newList new list
     */
    protected void switchSubscribersList(
            final List<Subscriber<T>> newList) {
        synchronized (subscribersLock) {
            newList.addAll(subscribers);
            subscribers.clear();
            subscribers = newList;
        }
    }
